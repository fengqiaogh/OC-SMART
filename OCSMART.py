...
from src.ocparam import CDOM, CHL, TSM
...
if "l1b_path" in input_param.keys():
    ...
    print(
        "\033[1;31;47mError: no L1B data directory, please set L1B_path in OCSMART_input.txt",
        "\033[m",
    )
    print("Quit OC-SMART ...")
    time.sleep(5)
...
if "l2_path" in input_param.keys():
    ...
    print(
        "Warning: L2 data directory not specified, using default L2 data directory: ./L2/"
    )
    L2_path = "./L2/"
####################################################
...
if isdir(L1_path):
    ...
    print(
        "Input level 1 data directory : {} does not exist, please check input file ...".format(
            L1_path
        )
    )
    print("Quit OC-SMART ...")
...
if isdir(L2_path):
    ...
    print(
        "Level 2 data directory: {} does not exist, creating level 2 data directory ... ".format(
            L2_path
        )
    )
    makedirs(L2_path)
...
for ifile in np.arange(nfiles):
    ...
    if sinfo.sensor_status == 0:
        ...
        if l1b.geoloc_status == 0:
            if (
                "north" in input_param.keys()
                and "south" in input_param.keys()
                and "east" in input_param.keys()
                and "west" in input_param.keys()
            ):
                ...
                "latitude_center" in input_param.keys()
                and "longitude_center" in input_param.keys()
                and "box_width" in input_param.keys()
                and "box_height" in input_param.keys()
            ):
                ...
                "start_line" in input_param.keys()
                and "end_line" in input_param.keys()
                and "start_pixel" in input_param.keys()
                and "end_pixel" in input_param.keys()
            ):
                ...
                l1b.latlon2linepixl(
                    start_line=sline, end_line=eline, start_pixel=spixl, end_pixel=epixl
                )
            else:
                l1b.sline = 0
                l1b.spixl = 0
            ...
            if l1b.lp_status == 0:
                ...
                lrc = np.zeros(l1b_dim, dtype="float64") - 999.0
                ...
                aods = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.aodnn_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                rrs = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.rrsnn_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                lrc_aann = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.aann_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                ...
                aph = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.aphnn_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                adg = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.adgnn_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                bbp = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.bbpnn_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                ap = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.apnn_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                bp = (
                    np.zeros(
                        [l1b_dim[0], l1b_dim[1], int(mlnn.bpnn_layers[-1])],
                        dtype="float64",
                    )
                    + np.nan
                )
                ...
                lat_delta = np.array(
                    [
                        np.abs(
                            l1b.latitude[midimg_x, midimg_y]
                ...
                lon_delta = np.array(
                    [
                        np.abs(
                            l1b.longitude[midimg_x, midimg_y]
                ...
                l1b_ray[mask_valid_geo_water, :] = (
                    l1b_ray[mask_valid_geo_water, :]
                    * press_fac[mask_valid_geo_water, :]
                )

                # Oxygen absorption correction for SeaWiFS band 7
                if sinfo.sensor == "SeaWiFS":
                    tg_o2[mask_valid_geo_water] = anc.trans_o2_ray(
                        l1b.solz[mask_valid_geo_water], l1b.senz[mask_valid_geo_water]
                    )
                    l1b_ray[mask_valid_geo_water, 6] = (
                        l1b_ray[mask_valid_geo_water, 6] * tg_o2[mask_valid_geo_water]
                    )
                ...
                if sinfo.sensor == "OLI":
                    ...
                    cmask[mask_valid_geo_water_lrcposi] = cm.run_cloudmask(
                        lrc[mask_valid_geo_water_lrcposi, :]
                    )
                ...
                #                    cmask[cmask_idx,cmask_idy] = True
                mask_valid_geo_water_lrcposi_cloud = (
                    mask_valid_geo_water_lrcposi & cmask
                )
                mask_valid_geo_water_lrcposi_nocloud = (
                    mask_valid_geo_water_lrcposi & ~cmask
                )
                ...
                if block_size < 0:
                    (
                        oos_flag[mask_valid_geo_water_lrcposi_nocloud],
                        lrc_aann[mask_valid_geo_water_lrcposi_nocloud, :],
                    ) = mlnn.compute_aann(
                    ...
                    print(
                        "Processing image in {} blocks ... ".format(
                            nblocks_x * nblocks_y
                        )
                    )
                    ...
                    if nblocks_x == 1:
                        ...
                        block_boundary_x[0:nblocks_x] = np.arange(
                            0, l1b_dim[1], block_size
                        )
                    ...
                    if nblocks_y == 1:
                        ...
                        block_boundary_y[0:nblocks_y] = np.arange(
                            0, l1b_dim[0], block_size
                        )
                    ...
                    for iby in np.arange(nblocks_y):
                        for ibx in np.arange(nblocks_x):
                            ...
                            blockmask[:, :] = False
                            blockmask[
                                block_boundary_y[iby] : block_boundary_y[iby + 1],
                                block_boundary_x[ibx] : block_boundary_x[ibx + 1],
                            ] = True
                            process_mask = (
                                mask_valid_geo_water_lrcposi_nocloud & blockmask
                            )
                            if np.sum(process_mask) > 0:
                                (
                                    oos_flag[process_mask],
                                    lrc_aann[process_mask, :],
                                ) = mlnn.compute_aann(
                                ...
                                aph[process_mask, :] = mlnn.compute_aphnn(
                                    rrs[process_mask, :]
                                )
                                adg[process_mask, :] = mlnn.compute_adgnn(
                                    rrs[process_mask, :]
                                )
                                bbp[process_mask, :] = mlnn.compute_bbpnn(
                                    rrs[process_mask, :]
                                )
                                ap[process_mask, :] = mlnn.compute_apnn(
                                    rrs[process_mask, :]
                                )
                                bp[process_mask, :] = mlnn.compute_bpnn(
                                    rrs[process_mask, :]
                                )

                mask_valid_geo_water_lrcposi_nocloud_oos = (
                    mask_valid_geo_water_lrcposi_nocloud & oos_flag
                )
                ...
                print(
                    "Writing level 2 file {} ... ".format(
                        os.path.splitext(fname)[0] + "_L2_OCSMART.h5"
                    )
                )
                hf = h5py.File(
                    L2_path + os.path.splitext(fname)[0] + "_L2_OCSMART.h5", "w"
                )
                hf.create_dataset(
                    "Latitude",
                    dtype="float32",
                ...
                print(
                    "\033[1;31;47mWARNING:Unable to extract subimage, processing terminated... ",
                    "\033[m",
                )
                continue
        else:
