...
class Rayleigh(object):
    def __init__(self, info=None):
        ...
        f.end()
        self.raytab = np.zeros(
            (nband, len(self.sigmatab), len(self.solztab), 3, len(self.senztab)),
            dtype="float64",
        )
        ...
        for i in range(nband):
            ...
            for j in range(nmod):
                ...
                func = interpolate.RegularGridInterpolator(
                    (self.sigmatab, self.solztab, self.senztab), tmp
                )
    ...
    def corr_ray_press(self, l1b_solz, l1b_senz, l1b_pressure):
        ...
        x = np.matmul(np.expand_dims(airmass, 1), [self.taur]) * (
            np.matmul(np.expand_dims(logam, 1), [x1]) - x2
        )
